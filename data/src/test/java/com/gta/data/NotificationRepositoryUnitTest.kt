...
class NotificationRepositoryUnitTest(
    ...
    fun Should_Success_When_saveNotificationWithGoodUid() = runBlocking {
        `when`(notificationDataSource.saveNotification(notification, eq(GOOD_UID), anyString())).thenReturn(flow { emit(true) })
    ...
    fun Should_FirestoreException_When_saveNotificationWithBadUid() = runBlocking {
        `when`(notificationDataSource.saveNotification(notification, anyString(), anyString())).thenReturn(flow { emit(false) })
